...
namespace YoutubeExplode
{
    public partial class YoutubeClient
    {
        ...
        private void ResolveClosedCaptionTrackInfos(string encodedData,
        ...
        public async Task<bool> CheckVideoExistsAsync(string videoId)
        {
            ...
            // Get video info
            var request = $"{YoutubeHost}/get_video_info" +
                          $"?video_id={videoId}" +
                          "&el=info" +
                          "&ps=default" +
                          "&hl=en";
        ...
        public async Task<Video> GetVideoAsync(string videoId)
        {
            ...
            var muxedStreamInfosEncoded = videoInfoDic.GetOrDefault("url_encoded_fmt_stream_map");
            if (muxedStreamInfosEncoded.IsNotBlank())
            {
                await ResolveMuxedStreamInfosAsync(context, muxedStreamInfosEncoded,
                        muxedStreamInfos)
            ...
            var adaptiveStreamInfosEncoded = videoInfoDic.GetOrDefault("adaptive_fmts");
            ...
            var closedCaptionTrackInfosEncoded = videoInfoDic.GetOrDefault("caption_tracks");
            if (closedCaptionTrackInfosEncoded.IsNotBlank())
            {
                ResolveClosedCaptionTrackInfos(closedCaptionTrackInfosEncoded,
            ...
            var description = videoInfoExtXml
                .ElementStrict("video_info").ElementStrict("description").Value;
            var likeCount = (long) videoInfoExtXml
                .ElementStrict("video_info").ElementStrict("likes_count_unformatted");
            var dislikeCount = (long) videoInfoExtXml
                .ElementStrict("video_info").ElementStrict("dislikes_count_unformatted");

            // Parse author info
            var authorId = videoInfoExtXml
                .ElementStrict("user_info").ElementStrict("channel_external_id").Value;
            var authorName = videoInfoExtXml
                .ElementStrict("user_info").ElementStrict("username").Value;
            var authorTitle = videoInfoExtXml
                .ElementStrict("user_info").ElementStrict("channel_title").Value;
            var authorIsPaid = videoInfoExtXml
                .ElementStrict("user_info").ElementStrict("channel_paid").Value == "1";
            var authorLogoUrl = videoInfoExtXml
                .ElementStrict("user_info").ElementStrict("channel_logo_url").Value;
            var authorBannerUrl = videoInfoExtXml
                .ElementStrict("user_info").ElementStrict("channel_banner_url").Value;
